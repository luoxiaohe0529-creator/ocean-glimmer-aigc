...
**大海浮光 AIGC 工作台**是一款面向电商与品牌内容团队的，能提供完整的 AI 广告生产流程的AIGC产品。系统内设计了四个专业 Agent，分别负责广告策划、编剧导演、摄影摄像和后期剪辑。其中，每个角色都配备了知识库作为高级智囊团。用户在agent生成创意方向后，只需把控与选择创意方向，就能生成复合要求的的AIGC视频。

在操作逻辑上，用户只需导入商品链接、产品文档或图片。全程无需编写提示词，也无需切换多个 AI 软件。系统先生成**产品简报、 和 12 条 Hook（每条hook都配备了创意方案、Mood Board情绪板）供用户选择与把控方向。**。方向确认后，脚本、分镜、视频和后期任务会在后端自动化执行，并将进度与结果实时回写。
